...
from genx.filehandling import create_string

from genx.models.lib.parameters import NumericParameter, get_parameters
...
class Model:
    ...
    def read_h5group(self, group, **kwargs):
        ...
        fom_func_name = create_string(group['fomfunction'].value)
    ...
    def save_addition(self, name, text):
        ...
        if name == 'data' or name == 'script' or name == 'parameters':
            raise IOError('It not alllowed to save a subfile with name: {0}'.format(name))
    ...
    def load_addition(self, name):
        ...
        try:
            ...
            raise IOError('Could not read the section named: {0}'.format(name), self.filename)
        loadfile.close()
        return text
    ...
    def create_fit_func(self, str):
        ...
        if isinstance(obj, NumericParameter):
            # We have a NumericParameter that should be set
            exec('def __tempfunc__(val):\n\t{0}.value = val'.format(str), self.script_module.__dict__)
        ...
        # Make a function of it
            ...
            exec('def __tempfunc__(val):\n\t{0} = val'.format(str), self.script_module.__dict__)
            return self.script_module.__tempfunc__
    ...
    def get_possible_set_functions(self):
        """Returns all the parameters that can be fitted given by the old style of defining
            parameters GenX2.4.X
