...
from genx.plugins.add_ons.help_modules.wxVTKRenderWindow import wxVTKRenderWindow
import genx.plugins.add_ons.help_modules.atom_colors as atom_colors
import genx.plugins.add_ons.help_modules.custom_dialog
import genx.plugins.add_ons.help_modules.sxrd_images as sxrd_images
...
class VTKview(wxVTKRenderWindow):
    ...
    def do_toolbar(self, parent):
        ...
        for i in range(len(button_names)):
            ...
            toolbar.AddCheckTool(new_id, button_names[i], button_images[i].GetBitmap(),
                                 shortHelp=tooltips[i])
            parent.Bind(wx.EVT_TOOL, callbacks[i], id=new_id)
