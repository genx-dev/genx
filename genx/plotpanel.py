...
class PlotPanel(wx.Panel):
    ...
    def __init__(self, parent, id = -1, color = None, dpi = None
            , style = wx.FULL_REPAINT_ON_RESIZE|wx.EXPAND|wx.ALL
...
#        DEBUG_MSG("__init__() - bitmap w:%d h:%d" % (w,h), 2, self)
    ...
    def ReadConfig(self):
        ...
        for index in range(len(bool_items)):
            try:
                val = self.config.get_boolean(self.config_name, bool_items[index])
...
class DataPlotPanel(PlotPanel):
    ...
    '''
    def __init__(self, parent, id = -1, color = None, dpi = None
    , style = wx.FULL_REPAINT_ON_RESIZE, **kwargs):
