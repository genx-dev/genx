...
__offspec__ = True
try:
    import genx.models.lib.offspec2_cython
except Exception as S:
    print('Failed to import: offspec2_cython, No off-specular simulations possible')
    print(S)
    __offspec__ = False
...
def OffSpecularMingInterdiff(TwoThetaQz, ThetaQx, sample, instrument):
    ...
    #print eta_z
    if __offspec__:
        (I, alpha, omega) = genx.models.lib.offspec2_cython.DWBA_Interdiff(qx, qz, lamda, n, z,\
...
if __name__=='__main__':
    Fe=Layer(d=10,sigmar=3.0,f=1-2.247e-5+2.891e-6j)
    Si=Layer(d=15,sigmar=3.0,f=1-7.577e-6+1.756e-7j)
    sub=Layer(sigmar=3.0,f=1-7.577e-6+1.756e-7j)
    amb=Layer(f=1.0,sigmar=1.0)
    stack=Stack(Layers=[Fe,Si],Repetitions=20)
    sample=Sample(Stacks=[stack],Ambient=amb,Substrate=sub,eta_z=500.0,eta_x=100.0)
    print(sample)
    inst=Instrument(wavelength=1.54,coords=1)
