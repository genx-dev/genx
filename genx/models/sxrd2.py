...
import genx.models.utils
from genx.models.symmetries import SymTrans, Sym
...
class Instrument:
    ...
    def __init__(self, inten=1.0, wavel=1.54, alpha=1.0, geom = 'alpha_in fixed', flib=None, rholib=None):
        ...
        if flib is None:
            self.flib = genx.models.utils.sl.FormFactor(wavel, genx.models.utils.__lookup_f__)
        else:
            self.flib = flib
        if rholib is None:
            self.rholib = genx.models.utils.sl.FormFactor(wavel, genx.models.utils.__lookup_rho__)
        else:
            self.rholib = rholib
...
if __name__ == '__main__':
    ...
    sample2 = Sample(domains=[domain2])
    ...
    f2 = sample2.calc_f(inst, h, k, l)
    import time
    t1 = time.time()
    sf = sample2.calc_f(inst, h, k, l)
    t2 = time.time()
    print('Python: %f seconds'%(t2-t1))
    t3 = time.time()    
    sft = sample2.turbo_calc_f(inst, h, k, l)
    t4 = time.time()
