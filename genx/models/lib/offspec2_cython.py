...
from genx.models.lib.elfield import *

offspec_ext_built = False
debug = False
try:
    import genx.models.lib.offspec_ext
...
def vec_realsymint(F, omega, eta, h, eta_z, qz_n, qz_np, sigma_n, sigma_np, max_n):
    I = 2 * sum([((qz_n * conj(qz_np) * (sigma_n * sigma_np)) ** n) / factorial(n) * eta / n ** (1.0 / 2.0 / h) * F(
        omega * eta / (n ** (1.0 / 2.0 / h))) for n in arange(1.0, max_n + 1.0)])
    I = array(I)
    print(I.shape)
    return I
...
def DWBA_Interdiff(qx, qz, lamda, n, z, sigma, sigmaid, eta, h, eta_z, d=[0],
    ...
    q_min = arange(min(abs(qx)) * eta / (maxn ** (1.0 / 2.0 / h)) - dqmin, max(abs(qx)) * eta + 2 * dqmin, dqmin,
                   dtype=float)
    ...
    s = genx.models.lib.offspec_ext.dwba_interdiff_sum(qx, G, q, eta, h, sigma, sigmaid,
...
def DWBA(qx, qz, lamda, n, z, sigma, eta, h, eta_z, d=[0], taylor_n=1):
    ...
    table = array(make_F(q_min, h), dtype=complex)
    fn = factorial(arange(1, maxn + 1))
    s = genx.models.lib.offspec_ext.dwba_sum(qx, G, q, eta, h, sigma, sqn, z, table, q_min, fn, eta_z)
    return (s, omega + omegap, omegap - omega)
...
def Born(qx, qz, lamda, n, z, sigma, eta, h, eta_z, d=[0], taylor_n=1):
    ...
    fn = factorial(arange(1, maxn + 1))
    s = genx.models.lib.offspec_ext.born_sum(qx, qz, eta, h, sigma, sqn, z, table, q_min, fn, eta_z)
...
if __name__ == '__main__':
    ...
    plt.plot(qx, real(I1), qx, real(I2))
    plt.show()
